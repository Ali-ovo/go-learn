...
import (
	"fmt"
	field2 "go-learn/project/v2/shop/pkg/common/validation/field"
	"net"
	"regexp"
	"strings"
	"unicode"
...
func IsValidIPv4Address(fldPath *field2.Path, value string) field2.ErrorList {
	var allErrors field2.ErrorList
	ip := net.ParseIP(value)
	if ip == nil || ip.To4() == nil {
		allErrors = append(allErrors, field2.Invalid(fldPath, value, "must be a valid IPv4 address"))
	}
	return allErrors
}

// IsValidIPv6Address tests that the argument is a valid IPv6 address.
func IsValidIPv6Address(fldPath *field2.Path, value string) field2.ErrorList {
	var allErrors field2.ErrorList
	ip := net.ParseIP(value)
	if ip == nil || ip.To4() != nil {
		allErrors = append(allErrors, field2.Invalid(fldPath, value, "must be a valid IPv6 address"))
	}
	return allErrors
}
