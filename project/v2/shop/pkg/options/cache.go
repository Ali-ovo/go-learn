...
type RedisOptions struct {
	...
	MaxIdle               int      `json:"max-idle"                 mapstructure:"max-idle"`                 // 最大空闲连接数
	MaxActive             int      `json:"max-active"               mapstructure:"max-active"`               // 最大连接次数
	TimeOut               int      `json:"time-out"                 mapstructure:"time-out"`                 // 连接超时时间
	MasterName            string   `json:"master-name"              mapstructure:"master-name"`              // 设置 Sentinel 分布式 redis 连接 主从架构
	EnableCluster         bool     `json:"enable-cluster"           mapstructure:"enable-cluster"`           // 是否是 Cluster 分布式 redis 连接 分片架构
	UseSSL                bool     `json:"use-ssl"                  mapstructure:"use-ssl"`                  // 是否支持 SSL连接 (redis 需要做对应的配置)
	SSLInsecureSkipVerify bool     `json:"ssl-insecure-skip-verify" mapstructure:"ssl-insecure-skip-verify"` // 是否跳过 SSL验证
	EnableTracing         bool     `json:"enable_tracing"           mapstructure:"enable-tracing"`           // 是否开启 tracing
}
