...
func GenerateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	if len(file.Services) == 0 { // 判断需要执行的文件是否大于0
		return nil
	}

	// 设置生成的文件名, 文件名会被 protoc 使用, 生成的文件会被放在响应的目录下
	filename := file.GeneratedFilenamePrefix + "_gin.pb.go" // 设置生成的文件名
	g := gen.NewGeneratedFile(filename, file.GoImportPath)  //  NewGeneratedFile 函数创建一个给定文件名和导入路径的新生成文件
	// 该注释会被 go 的ide 识别到, 表示该文件是自动生成的, 尽量不要修改
	g.P("// Code generated by protoc-gen-gin. DO NOT EDIT.")
	g.P()
	g.P("package ", file.GoPackageName)

	// 该函数会注册全局的 packge 的内容, 但是此时不会写入文件, 只是生成一个包的声明
	g.QualifiedGoIdent(ginPkg.Ident(""))
	g.QualifiedGoIdent(httpPkg.Ident(""))

	for _, service := range file.Services {
		genService(file, g, service) // 通过模板生成后的数据 写入到 g 中 后生成文件
	}
...
func genService(file *protogen.File, g *protogen.GeneratedFile, s *protogen.Service) {
	// HTTP Server
	sd := &service{ // 设置模板所需要的数据
		Name:     s.GoName,                  // Greeter
		FullName: string(s.Desc.FullName()), // helloworld.v1.Greeter
	}

	// 将 额外内容 复制给 sd.Methods
