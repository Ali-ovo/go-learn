...
type method struct {
	...
	// http rule
	Path         string
	PathParams   []string // 类型 比如 int类型 等
	Method       string
	Body         string
	ResponseBody string
}
...
func (m *method) initPathParams() {
	paths := strings.Split(m.Path, "/")
	for i, p := range paths { // 如果为空 并且 (开头和结尾 有 {} 或 p[0] 为 :)
...
// GoCamelCase 将一个 Protobuf 名称转为驼峰式的 Go 标识符。
// 如果名称中有一个下划线后面跟着一个小写字母，则删掉下划线并将字母转为大写。
// 该函数源自 protobuf/internal/strings.go。
