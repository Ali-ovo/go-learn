...
// shouldConnect 读取 disableRedis 的值
//
//	@Description:
//	@return bool
...
again:
	for {
		select {
		// 如果 调用 此ctx 的 cancel()  说明 断开 redis 连接
		case <-ctx.Done():
			return
		// 间隔 1s 执行以下操作
		case <-tick.C:
			...
			for _, v := range c {
				// 前面已经创建了 redis conn 所以直接返回 true
				if !connectSingleton(v.IsCache, config) {
					redisUp.Store(false)
...
// NewRedisClusterPool create a redis cluster pool.	创建 一个 redis 连接池
//
//	@Description:
//	@param isCache
//	@param config
...
func NewRedisClusterPool(isCache bool, config *Config) redis.UniversalClient {
	...
	if config.MaxActive > 0 {
		poolSize = config.MaxActive // 设置最大连接数
	}

	timeout := 5 * time.Second

	if config.Timeout > 0 {
		timeout = time.Duration(config.Timeout) * time.Second // 设置超时时间
	}
	...
	if opts.MasterName != "" {
		...
		// 创建 redis Sentinel 分布式 redis 主节点负责写操作 从节点负责读操作 主从架构
