...
func (gs *goodsService) List(ctx context.Context, req *goods_pb.GoodsFilterRequest, orderby []string) (*dto.GoodsDTOList, error) {
	...
	if req.TopCategory > 0 {
		category, err := gs.data.Category().Get(ctx, req.TopCategory)
		if err != nil {
	...
	goodsData, err := gs.data.Goods().ListByIDs(ctx, goodsIDs, orderby)
	if err != nil {
		return nil, err
	}
...
func (gs *goodsService) Create(ctx context.Context, goods *dto.GoodsDTO) (int32, error) {
	/*
	...
	*/
	var err error
	if _, err = gs.data.Brands().Get(ctx, goods.BrandsID); err != nil {
		return 0, err
	}
	if _, err = gs.data.Category().Get(ctx, goods.CategoryID); err != nil {
		return 0, err
	}
